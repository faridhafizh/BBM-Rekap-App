document.addEventListener('DOMContentLoaded', () => {
    ...
    btnLoadSheets.addEventListener('click', async () => {
        ...
        try {
            ...
            alert("Terjadi kesalahan koneksi. Pastikan URL benar dan CORS diizinkan di Apps Script.");
            console.error(error);
        } finally {
            hideLoader();
        }
    ...
    btnSearch.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${url}?action=search&sheetName=${encodeURIComponent(sheet)}&id=${encodeURIComponent(id)}`);
            ...
            if (result.status === 'success' && result.data) {
                ...
                showMessage('Data ditemukan! Silakan edit jika perlu.', true);
            } else {
                // Not found, clean form for new entry
                const currentType = menuType.value;
                resetFormFields();
                menuType.value = currentType; // preserve selected type if any
                menuType.dispatchEvent(new Event('change'));
                showMessage('ID tidak ditemukan di database. Akan disimpan sebagai data baru.', true);
            }
        } catch (error) {
            console.error(error);
            showMessage('Gagal mencari data.', false, true);
        } finally {
            hideLoader();
        }
    });
    ...
    btnGenerate.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${url}?action=getAll&sheetName=${encodeURIComponent(sheet)}`);
            const result = await response.json();
